...
int janus_process_incoming_request(janus_request *request) {
	...
	if(session_id == 0 && handle_id == 0) {
		...
		json_object_set_new(data, "handle_id", json_integer(handle_id));
...
void janus_transport_task(gpointer data, gpointer user_data) {
	JANUS_LOG(LOG_INFO, "Transport task pool, serving request\n");
...
json_t *janus_plugin_handle_sdp(janus_plugin_session *plugin_session, janus_plugin *plugin, const char *sdp_type, const char *sdp, gboolean restart) {
    ...
    JANUS_LOG(LOG_INFO, "sdp merged %s\n", sdp_merged);
...
gint main(int argc, char *argv[])
{
	...
	if(janus_log_init(daemonize, TRUE, NULL) < 0)
		exit(1);

	JANUS_PRINT("Starting Media Server(author email:765063568@qq.com)\n");
	...
	janus_config_item *item = janus_config_get_item_drilldown(config, "general", "session_timeout");
	if(item && item->value) {
